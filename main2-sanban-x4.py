...
# 计算定位坐标
def triposition_d(A0, da, A1, db, A2, dc, A4, dd):
...
def triposition_c(A0, da, A1, db, A2, dc, A4, dd):
    xa, ya, za = A0[0], A0[1], A0[2]
    xb, yb, zb = A1[0], A1[1], A1[2]
    xc, yc, zc = A2[0], A2[1], A2[2]
    xd, yd, zd = A3[0], A3[1], A3[2]
    ...
    x, y, z = sympy.symbols("x, y, z")
    f1 = 2 * x * (xa - xc) + np.square(xc) - np.square(xa) + 2 * y * (ya - yc) + np.square(yc) - np.square(ya) + 2 * z * (za - zc) + np.square(zc) - np.square(za) - (np.square(dc) - np.square(da))
    f2 = 2 * x * (xb - xc) + np.square(xc) - np.square(xb) + 2 * y * (yb - yc) + np.square(yc) - np.square(yb) + 2 * z * (zb - zc) + np.square(zc) - np.square(zb) - (np.square(dc) - np.square(db))
    f3 = 2 * x * (xd - xc) + np.square(xc) - np.square(xd) + 2 * y * (yd - yc) + np.square(yc) - np.square(yd) + 2 * z * (zd - zc) + np.square(zc) - np.square(zd) - (np.square(dc) - np.square(dd))
    result = sympy.solve([f1, f2, f3], [x, y, z])

    locx, locy, locz = result[x], result[y], result[z]
    return [locx, locy, locz]

def triposition_b(A0, da, A1, db, A2, dc, A4, dd):
    xa, ya, za = A0[0], A0[1], A0[2]
    xb, yb, zb = A1[0], A1[1], A1[2]
    xc, yc, zc = A2[0], A2[1], A2[2]
    xd, yd, zd = A3[0], A3[1], A3[2]
    ...
    x, y, z = sympy.symbols("x, y, z")
    f1 = 2 * x * (xa - xb) + np.square(xb) - np.square(xa) + 2 * y * (ya - yb) + np.square(yb) - np.square(ya) + 2 * z * (za - zb) + np.square(zb) - np.square(za) - (np.square(db) - np.square(da))
    f2 = 2 * x * (xc - xb) + np.square(xb) - np.square(xc) + 2 * y * (yc - yb) + np.square(yb) - np.square(yc) + 2 * z * (zc - zb) + np.square(zb) - np.square(zc) - (np.square(db) - np.square(dc))
    f3 = 2 * x * (xd - xb) + np.square(xb) - np.square(xd) + 2 * y * (yd - yb) + np.square(yb) - np.square(yd) + 2 * z * (zd - zb) + np.square(zb) - np.square(zd) - (np.square(db) - np.square(dd))
    result = sympy.solve([f1, f2, f3], [x, y, z])

    locx, locy, locz = result[x], result[y], result[z]
    return [locx, locy, locz]

def triposition_a(A0, da, A1, db, A2, dc, A4, dd):
    xa, ya, za = A0[0], A0[1], A0[2]
    xb, yb, zb = A1[0], A1[1], A1[2]
    xc, yc, zc = A2[0], A2[1], A2[2]
    xd, yd, zd = A3[0], A3[1], A3[2]
    ...
    x, y, z = sympy.symbols("x, y, z")
    f1 = 2 * x * (xb - xa) + np.square(xa) - np.square(xb) + 2 * y * (yb - ya) + np.square(ya) - np.square(yb) + 2 * z * (zb - za) + np.square(za) - np.square(zb) - (np.square(da) - np.square(db))
    f2 = 2 * x * (xc - xa) + np.square(xa) - np.square(xc) + 2 * y * (yc - ya) + np.square(ya) - np.square(yc) + 2 * z * (zc - za) + np.square(za) - np.square(zc) - (np.square(da) - np.square(dc))
    f3 = 2 * x * (xd - xa) + np.square(xa) - np.square(xd) + 2 * y * (yd - ya) + np.square(ya) - np.square(yd) + 2 * z * (zd - za) + np.square(za) - np.square(zd) - (np.square(da) - np.square(dd))
    result = sympy.solve([f1, f2, f3], [x, y, z])
...
for i in range(324):
    ...
    res, err = [0, 0, 0, 0], [0, 0, 0, 0]
    res[0] = triposition_a(A0, A_dis[0], A1, A_dis[1], A2, A_dis[2], A3, A_dis[3])
    res[1] = triposition_b(A0, A_dis[0], A1, A_dis[1], A2, A_dis[2], A3, A_dis[3])
    res[2] = triposition_c(A0, A_dis[0], A1, A_dis[1], A2, A_dis[2], A3, A_dis[3])
    res[3] = triposition_d(A0, A_dis[0], A1, A_dis[1], A2, A_dis[2], A3, A_dis[3])

    err[0] = disErr_scene1(res[0], A_dis)
    err[1] = disErr_scene1(res[1], A_dis)
    err[2] = disErr_scene1(res[2], A_dis)
    err[3] = disErr_scene1(res[3], A_dis)

    min_index = np.argmin(np.array(err))

    global_best_x_list_R.append(res[min_index])
    global_best_y_list_R.append(err[min_index])
    print(i, '正常: best_x:', res[min_index], 'best_y:', err[min_index])
    ...
    global_best_x_list_W_e1.append(res[min_index])
    global_best_y_list_W_e1.append(err[min_index])
    print(i, '异常-1: best_x:', res[min_index], 'best_y:', err[min_index])
    ...
    global_best_x_list_W_e2.append(res[min_index])
    global_best_y_list_W_e2.append(err[min_index])
    print(i, '异常-2: best_x:', res[min_index], 'best_y:', err[min_index])
...
df_out.to_excel('./输出/第二问遗传算法分类-sanbian-x4.xlsx')
